...
const findPasswordSubmit = async () => {
    if (!Object.values(findPasswordForm.value).every(item => item)) return proxy.$toast.show('请输入完整信息')
    try {
        proxy.$toast.loading('重置中')
        await resetPassword(findPasswordForm.value)
        LoginStore.findPasswordShow = false
        proxy.$toast.show('密码重置成功')
        resetForm()
    } catch (error) {
        if (error.response.data.message === 'code_error') return proxy.$toast.show('验证码错误')
        proxy.$toast.show('密码重置失败')
    }
}

const GetfindPasswordCode = async () => {
    if (!findPasswordForm.value.email) return proxy.$toast.show('请输入邮箱')
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(findPasswordForm.value.email)) return proxy.$toast.show('请输入正确的邮箱')
    try {
        ...
        if (error.response.data.message === 'user not found by email.') return proxy.$toast.show('邮箱不存在')
        proxy.$toast.show('获取验证码失败')
    }
}
</script>
